...
export async function addToBasket(req: express.Request, res: express.Response) {
    try {
        ...
        let basketItem: any = (
            await client.query(`select * from baskets where ordered_by = $1 and  product_id = $2`, [userId, productId])
        ).rows[0];
        ...
        if (basketItem) {
            ...
            await client.query(`update baskets set quantity = $1 where ordered_by = $2`, [
                Number(quantity) + basketItem.quantity,
                userId,
            ]);
        } else {
